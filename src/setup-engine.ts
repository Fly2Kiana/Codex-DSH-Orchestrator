...
export async function atomicInstallText(options: AtomicInstallTextOptions): Promise<AtomicInstallTextResult> {
  ...
  try {
    ...
    // Windows ACLs are authoritative and Node cannot round-trip POSIX chmod bits there:
    // a requested 0600/0640 file is commonly reported as 0666. Regular-file and
    // symlink checks still run through readConfigSnapshot, while Unix keeps the
    // exact mode-preservation requirement.
    const modeMatches = process.platform === "win32" || installed.mode === latest.mode;
    if (!installed.exists || installed.content !== options.content || !modeMatches || !verified) {
      throw verificationError(backupPath);
    }
