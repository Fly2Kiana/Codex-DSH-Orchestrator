...
export async function collectLockDiagnostics(homeDir: string): Promise<LockDiagnostics> {
  const locks: LockDiagnostic[] = [
    await diagnoseLock(join(homeDir, "claims", "registry.lock"), "claims registry.lock", "registry"),
    await diagnoseLock(join(homeDir, "tasks", "registry.lock"), "tasks registry.lock", "registry"),
  ];
