...
interface RenderMcpConfigBaseOptions {
  entryPath: string;
  nodePath: string;
  dshVersion?: string;
  preset?: string;
...
export type RenderMcpConfigOptions = RenderMcpConfigBaseOptions &
  (
    | {
        hostUrl: string;
        hostMode?: "static";
...
export type CodexInstallPlanOptions = RenderMcpConfigOptions & {
  configPath: string;
  replace: boolean;
};
