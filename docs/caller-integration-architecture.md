# Multi-caller extension architecture for Codex-DSH-Orchestrator

Status: Implemented baseline with explicitly deferred extensions
This English document is authoritative; the Chinese version should remain semantically aligned with it.

## 1. Decision summary

Codex-DSH-Orchestrator uses one repository, one shared dsh-Agentlink-derived runtime, and multiple Caller Integration Packs. Codex, Claude Code, ZCode, Workbuddy, and other MCP-capable callers should invoke the same Agentlink MCP Runtime instead of duplicating DSH session, event, approval, and recovery logic.
...
Claude Code was the first new caller used to validate this extension architecture. It is not a new runtime or a separate product branch. Its caller-specific integration remains limited to an Integration Pack and excludes a Gateway and any `claude -p` wrapper; the shared MCP Runtime now exposes the same existing-session discovery and attachment tools to every configured MCP caller.
...
- Caller integrations expose only catalog-validated semantic model profiles and reasoning effort; arbitrary provider/model strings remain unsupported, and omitted routing inherits the DSH route.
- Existing-session attachment is a shared MCP Runtime capability. Caller-specific cross-caller takeover UI and a resident Gateway remain out of scope.
...
Creating a new task and attaching to an existing DSH session remain separate use cases. `dsh_delegate` never accepts an optional session id. Existing-session attachment uses two explicit tools:

- `dsh_find_sessions` reads bounded root-session metadata only and never conversation history or raw projections.
- `dsh_attach_session` requires an exact session id plus fresh update/cwd/title preconditions and accepts only an idle root. Claim rejection rolls back a mapping created by that attachment before supervision begins; existing mappings are preserved. This is operation-failure atomicity, not hard-crash atomicity across two files.
- Titles assist discovery but never select the session. Stale metadata, descendants, running sessions, workspace conflicts, and duplicate legacy mappings fail closed.
- Attachment itself does not prompt, rename, create a session, or change the model. `dsh_followup` is the separate resume/write boundary.
- Releasing the workspace claim closes only bridge coordination; it does not close the DSH session. Turn cancellation and DSH session lifecycle remain separate operations.
- Cooperative claims coordinate callers sharing one bridge home; DSH Web and other bridge homes remain outside enforcement.
...
`dsh_delegate` and `dsh_followup` expose only semantic model profiles and a catalog-validated reasoning effort, not arbitrary provider/model strings. Omitting routing inherits the current DSH route. An explicit user choice always wins; any autonomous caller selection must be disclosed, validated against live `session.models`, re-read before the prompt, and report the DSH rc.6 global-default persistence side effect. Attachment itself never changes model routing; routed follow-up is the separate explicit write boundary.
...
- Do not wrap `claude -p`, add a Claude-specific attachment implementation, or start `dsh web`; shared MCP attachment remains caller-neutral.

### Phase 3: a second new caller

**Status: deferred; ZCode remains the first candidate when caller expansion resumes.**
...
- No `claude -p` wrapper, Claude-specific session attachment path, Gateway, or DSH lifecycle management; shared MCP attachment tools remain available.
...
- Gateway, ACP, and cross-caller task discovery/transfer remain deferred behind explicit triggers; exact existing-session attachment in one caller is implemented.
...
Deferred and undecided: Gateway transport/authentication, ACP packaging, cross-caller task visibility/transfer, external third-party Integration Packs, executable third-party session-launch adapters, and an independent npm workspace. Exact existing-session discovery and attachment through the shared MCP Runtime are implemented and are no longer part of this deferred list. See [Deferred roadmap](deferred-roadmap.md) for activation triggers and preserved boundaries.
