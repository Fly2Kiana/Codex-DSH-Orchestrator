# Codex-DSH-Orchestrator 多调用方扩展架构
...
Codex-DSH-Orchestrator 采用一个仓库、一个共享的 dsh-Agentlink 派生运行时和多个调用方集成包。Codex、Claude Code、ZCode、Workbuddy 等调用方只要能够使用 MCP，就应调用同一个 Agentlink MCP Runtime，而不是各自复制一套 DSH 会话、事件、审批和恢复逻辑。
...
Claude Code 是用于验证这套扩展架构的第一个新增调用方，不是新的运行时，也不是新的产品分支。其调用方专用接入仍只做 Integration Pack，不包含 Gateway 或 `claude -p` 包装；共享 MCP Runtime 现在会向每个已配置的 MCP 调用方提供相同的既有 session 发现与接管工具。
...
- 调用方集成只暴露经过 catalog 校验的语义模型档位与 reasoning effort；仍不接受任意 provider/model 字符串，省略路由时继承 DSH 当前 route。
- 既有 session 接管属于共享 MCP Runtime 能力；调用方专用的跨调用方接管 UI 与常驻 Gateway 仍不在范围内。
...
“新建任务”和“接入已有 DSH session”仍是两个独立用例。`dsh_delegate` 不接受可选 session id。已有会话接管使用两个显式工具：

- `dsh_find_sessions` 只读取有界 root-session 元数据，不读取对话 history 或 raw projections；
- `dsh_attach_session` 要求精确 session id 加新鲜的 update/cwd/title 前置条件，只接受空闲 root；claim 被拒绝时，会在 supervision 开始前回滚本次接管新建的 mapping，并保留既有 mapping。这是普通操作失败的原子性，不是跨两个文件的硬崩溃原子性；
- 标题只辅助发现，绝不选择 session。过期元数据、descendant、运行中 session、workspace conflict 与旧版重复 mapping 都会失败关闭；
- 接管本身不会 prompt、rename、创建 session 或改变模型；`dsh_followup` 是单独的 resume/write 边界；
- 释放 workspace claim 只关闭 bridge coordination，不关闭 DSH session；turn cancel 与 DSH session lifecycle 仍是不同操作；
- cooperative claim 只协调共享同一 bridge home 的调用方；DSH Web 与其他 bridge home 不在执行范围内。
...
`dsh_delegate` 与 `dsh_followup` 只暴露语义模型档位与 catalog 校验的 reasoning effort，不接受任意 provider/model 字符串。省略路由时继承 DSH 当前 route。用户显式选择永远优先；调用方自主选择时必须披露原因、按实时 `session.models` 验证、在 prompt 前重新读取确认，并报告 DSH rc.6 持久化全局默认值的副作用。接管本身不会改变模型路由；routed follow-up 是单独且显式的写入边界。
...
- 不包装 `claude -p`，不增加 Claude 专用接管实现，也不启动 `dsh web`；共享 MCP 接管能力保持 caller-neutral。

### Phase 3：第二个新增调用方

**状态：已延期；恢复调用方扩展时仍优先考虑 ZCode。**
...
- Codex 安装器与完整测试保持通过。
- 没有 `claude -p` wrapper、Claude 专用 session attach 路径、Gateway 或 DSH 生命周期管理；共享 MCP 接管工具仍可使用。
...
- Gateway、ACP 与跨调用方 task 发现/转交有明确触发条件且保持延期；单一调用方内按精确 session 接管已经实现。
...
延期且未决定：Gateway transport/auth、ACP packaging、跨调用方 task visibility/transfer、外部第三方 Integration Pack、可执行的第三方 session-launch adapter、独立 npm workspace。共享 MCP Runtime 的精确既有 session 发现与接管已经实现，不再属于延期项。启动条件与保留边界见[延期路线图](deferred-roadmap.zh-CN.md)。
