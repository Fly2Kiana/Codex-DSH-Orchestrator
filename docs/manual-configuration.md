...
Delegation and follow-up accept only the semantic `inherit|flash|pro|modlens-flash|modlens-pro` profiles plus a live-catalog-supported reasoning effort; arbitrary provider/model strings are rejected. Omitting routing inherits and verifies DSH's current route. Explicit selection can persist as the DSH global default. `workspaceMode` is a bridge-local cooperative claim only; it does not select, enforce, or verify the DSH Host filesystem sandbox.
