...
import { assertModePreserved, assertPortablePrivateMode } from "./support/platform.js";
...
test("Claude Code setup installs .mcp.json atomically and preserves its observable mode", async (context) => {
  ...
  const modeBefore = (await stat(configPath)).mode;
  ...
  assertModePreserved(modeBefore, (await stat(configPath)).mode);
...
test("Claude Code setup installs the project skill with a private POSIX mode where supported", async (context) => {
  ...
  assertPortablePrivateMode((await stat(skillPath)).mode, 0o600);
});
...
test("Claude Code setup replaces a different project skill only with --replace-skill", async (context) => {
  ...
  const modeBefore = (await stat(skillPath)).mode;
  ...
  assertModePreserved(modeBefore, (await stat(skillPath)).mode);
});
