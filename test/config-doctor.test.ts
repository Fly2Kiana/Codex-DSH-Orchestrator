...
test("config enables opt-in Desktop discovery while an explicit Host URL remains authoritative", () => {
  const base = { DSH_HOME: "/tmp/dsh-test-home", DSH_BRIDGE_TIME_ZONE: "UTC" };
  const desktop = loadConfig({ ...base, DSH_HOST_MODE: "desktop-auto" });
  assert.equal(desktop.hostMode, "desktop-auto");

  const explicit = loadConfig({
    ...base,
    DSH_HOST_MODE: "desktop-auto",
    DSH_HOST_URL: "http://127.0.0.1:43210",
  });
  assert.equal(explicit.hostMode, "static");
  assert.equal(explicit.hostUrl, "http://127.0.0.1:43210");

  assert.throws(() => loadConfig({ ...base, DSH_HOST_MODE: "scan-all-ports" }), /DSH_HOST_MODE/);
});
...
test("lock diagnostics include the TaskStore reverse-mapping registry lock", async () => {
  const homeDir = await makeHome();
  try {
    await writeLock(homeDir, "tasks/registry.lock", ownerFile("SECRET-task-registry-token", 5151));

    const diagnostics = await collectLockDiagnostics(homeDir);
    const registry = diagnostics.locks.find((lock) => lock.label === "tasks registry.lock")!;
    assert.equal(registry.presence, "present");
    assert.equal(registry.ownerPresence, "present");
    assert.equal(JSON.stringify(diagnostics).includes("SECRET-task-registry-token"), false);
  } finally {
    await fs.rm(homeDir, { recursive: true, force: true });
  }
});
