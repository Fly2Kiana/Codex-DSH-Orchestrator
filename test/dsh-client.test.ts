...
test("a transport failure invalidates a resolved endpoint without retrying the failed write", async () => {
  const first = await startMockDshHost();
  const second = await startMockDshHost();
  let resolveCalls = 0;
  try {
    first.setUnaryHandler("host.describe", () => ({
      version: "first",
      cwd: "/tmp/first",
      attachedSessions: 0,
      canOpenPath: true,
    }));
    second.setUnaryHandler("host.describe", () => ({
      version: "second",
      cwd: "/tmp/second",
      attachedSessions: 0,
      canOpenPath: true,
    }));
    second.setUnaryHandler("session.prompt", () => ({ accepted: true }));
    const endpoints = [first.baseUrl, second.baseUrl];
    const client = new DshClient(
      {
        mode: "desktop-auto",
        resolve: async () => ({
          baseUrl: endpoints[Math.min(resolveCalls++, endpoints.length - 1)]!,
    ...
    assert.equal((await client.hostDescribe()).version, "first");
    ...
    await assert.rejects(
      () =>
        client.sessionPrompt({
          sessionId: "session-1",
          mode: "queue",
          content: [{ type: "text", text: "must not be retried" }],
        }),
      DshTransportError,
    );
    assert.equal(resolveCalls, 1);
    ...
    assert.equal((await client.hostDescribe()).version, "second");
    assert.equal(resolveCalls, 2);
    assert.deepEqual(client.endpointSnapshot(), {
      mode: "desktop-auto",
      resolvedBaseUrl: second.baseUrl,
      source: "windows-process-listener",
      resolutionEpoch: 2,
    });
  } finally {
...
test("session.selectModel sends the exact route and parses the selected model", async () => {
  const host = await startMockDshHost();
  try {
    host.setUnaryHandler("session.selectModel", () => ({
      selected: {
    ...
    const result = await clientFor(host.baseUrl).sessionSelectModel({
      sessionId: "session-1",
      provider: "deepseek-modlens",
      model: "deepseek-v4-pro",
      reasoningEffort: "high",
    ...
    assert.equal(host.requests[0]?.method, "session.selectModel");
    assert.deepEqual(host.requests[0]?.payload, {
      sessionId: "session-1",
      provider: "deepseek-modlens",
      model: "deepseek-v4-pro",
    ...
    assert.equal(getDshUnaryMetadata(result).method, "session.selectModel");
  } finally {
    await host.close();
  }
});
