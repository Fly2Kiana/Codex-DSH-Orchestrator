...
import {
  ...
  parseSetupArgs,
  readConfigSnapshot,
  requireExplicitBridgeReplacement,
  renderCodexOperation,
  renderMcpConfig,
  upsertMcpConfig,
  validateDshCliSetupPrerequisite,
  validateDesktopAutoPlatform,
...
const desktopAutoBlock = renderMcpConfig({
  nodePath: "/Applications/Node Runtime/bin/node",
  entryPath: "/tmp/dsh Agentlink/dist/index.js",
  hostMode: "desktop-auto",
  preset: "code",
});
...
test("setup renders desktop auto mode without a stale static Host URL", () => {
  assert.match(desktopAutoBlock, /DSH_HOST_MODE = "desktop-auto"/);
  assert.equal(desktopAutoBlock.includes("DSH_HOST_URL"), false);
  assert.equal(desktopAutoBlock.includes("DSH_HOST_VERSION"), false);
  assert.match(desktopAutoBlock, /DSH_BRIDGE_AGENT_PRESET = "code"/);
  assert.match(desktopAutoBlock, /approval_mode = "prompt"/);
  ...
  assert.deepEqual(operation.env, { DSH_HOST_MODE: "desktop-auto" });
});
...
test("setup requires the --replace decision before any existing bridge can be updated", () => {
  const current = `[mcp_servers.dsh_agentlink]\ncommand = "old"\n`;
  const legacy = `[mcp_servers.dsh_collab]\ncommand = "old"\n`;

  assert.throws(() => requireExplicitBridgeReplacement(current, false), /rerun with --replace/);
  assert.throws(() => requireExplicitBridgeReplacement(legacy, false), /rerun with --replace/);
  assert.doesNotThrow(() => requireExplicitBridgeReplacement(current, true));
  assert.doesNotThrow(() => requireExplicitBridgeReplacement(`[mcp_servers.other]\ncommand = "keep"\n`, false));
});

test("desktop auto setup treats a missing DSH CLI version as a warning-only condition", () => {
  assert.equal(validateDshCliSetupPrerequisite("desktop-auto", undefined), undefined);
  assert.equal(validateDshCliSetupPrerequisite("desktop-auto", "0.1.0-rc.6"), "0.1.0-rc.6");
  assert.equal(validateDshCliSetupPrerequisite("static", "0.1.0-rc.6"), "0.1.0-rc.6");
  assert.throws(() => validateDshCliSetupPrerequisite("static", undefined), /DSH CLI was not found/);
});

test("desktop auto setup rejects unsupported operating systems before installation", () => {
  assert.doesNotThrow(() => validateDesktopAutoPlatform("desktop-auto", "win32"));
  assert.doesNotThrow(() => validateDesktopAutoPlatform("static", "linux"));
  assert.throws(() => validateDesktopAutoPlatform("desktop-auto", "linux"), /supported only on Windows/);
});

test("setup resolves the Codex config root without repurposing the environment", () => {
  const customRoot = join(tmpdir(), "custom-codex");
  assert.equal(defaultCodexConfigPath({ CODEX_HOME: customRoot }), join(customRoot, "config.toml"));
  assert.equal(
    codexIntegration.defaultConfigPath({ cwd: join(tmpdir(), "project"), env: { CODEX_HOME: customRoot } }),
    join(customRoot, "config.toml"),
  );
});
...
test("setup backs up an existing config and preserves private file modes", async (context) => {
  ...
  const observedMode = (await stat(configPath)).mode & 0o777;
  const expected = await readConfigSnapshot(configPath);
  ...
  assert.equal((await stat(configPath)).mode & 0o777, observedMode);
  assert.equal((await stat(result.backupPath as string)).mode & 0o777, observedMode);
