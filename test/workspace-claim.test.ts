...
test("WorkspaceClaimStore persists only coordination fields with private POSIX modes where supported", async () => {
  ...
  try {
    ...
    assertPortablePrivateMode((await stat(join(home, "claims"))).mode, 0o700);
