...
export class FakeDshApi implements DshApi {
  ...
  async sessionSelectModel(payload: { sessionId: string; provider: string; model: string; reasoningEffort?: string }) {
    this.calls.push({ method: "session.selectModel", payload });
    const selected = {
      provider: payload.provider,
      model: payload.model,
      ...(payload.reasoningEffort === undefined ? {} : { reasoningEffort: payload.reasoningEffort }),
    };
    this.models = { ...this.models, current: selected };
    return this.unary("session.selectModel", { selected });
  }
