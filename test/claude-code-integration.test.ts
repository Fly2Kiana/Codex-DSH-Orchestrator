...
test("Claude Code integration exposes project-scoped install plan without original config secrets", () => {
  ...
  assert.deepEqual(plan.target, {
    path: join("/tmp/project", ".mcp.json"),
    format: "json",
    scope: "project",
  });
...
test("Claude Code integration can include a native project skill operation", () => {
  ...
  assert.deepEqual(skill, {
    ...
    targetPath: join("/tmp/project", ".claude", "skills", "claude-code-dsh", "SKILL.md"),
    content: "---\nname: claude-code-dsh\n---\n",
...
test("Claude Code setup path is project local", () => {
  ...
  assert.equal(
    claudeCodeIntegration.defaultConfigPath({ cwd: "/tmp/project", env: {} }),
    join("/tmp/project", ".mcp.json"),
  );
